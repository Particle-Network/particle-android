...
class ParticleWalletLoginDemoActivity : DemoBaseActivity<ActivityParticleWalletDemoBinding>(R.layout.activity_particle_wallet_demo) {
    val pageTipsStr = arrayOf(R.string.pn_page1_tips, R.string.pn_page2_tips, R.string.pn_page3_tips, R.string.pn_page4_tips)
...
//        }
    ...
    private fun initHorizontalBanner() {
        binding.bannerView.setScrollDuration(600).setOffScreenPageLimit(2).setLifecycleRegistry(lifecycle).setIndicatorStyle(IndicatorStyle.CIRCLE)
            .setIndicatorSlideMode(IndicatorSlideMode.NORMAL).setInterval(3000).setIndicatorGravity(IndicatorGravity.CENTER)
            .setIndicatorSliderRadius(10).disallowParentInterceptDownEvent(true).setIndicatorSliderColor(
                Color.parseColor("#4E4E50"), Color.WHITE
            ).setAdapter(BannerAdapter()).registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
    ...
    private fun loginWithPn(
        ...
        ParticleNetwork.login(loginType, supportAuthType.value, false, loginCallback = object : WebServiceCallback<LoginOutput> {
    ...
    private fun walletConnect() {
        ...
        qrFragment.setConnectCallback(object : WallectConnectTabCallback {
            override fun onConnect(account: Account, wallectType: WalletType) {
                ...
                lifecycleScope.launch {
                    val wallet = WalletUtils.createSelectedWallet(account.publicAddress, wallectType)
                    ParticleWallet.setWallet(wallet)
                    openWallet()
                }
            }
    ...
    override fun setObserver() {
        launcherResult = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { activityResult ->
